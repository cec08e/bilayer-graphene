...
the lattice contains two carbon atoms, which we will label $A$ and $B$. From each carbon atom, we only consider a single $2p_{z}$ orbital. As described in the first section, the remaining valence electrons are tied
...
$$\bra{\phi_{A}}\Delta U_{A}\ket{\psi_{k}} = $$
$$\bra{\phi_{B}}\Delta U_{B}\ket{\psi_{k}} = $$
